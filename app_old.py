...
def soil_layer(df):
    ...
    if ndf.empty == False:
        ndf['#200 (%)'] = ndf['#200 (%)'].fillna(method='ffill')
        ndf['#200 (%)'] = ndf['#200 (%)'].fillna(100)
        ndf.loc[(ndf['#200 (%)'] >= 50), 'Soil type'] = 0  # clay
        ndf.loc[(ndf['#200 (%)'] < 50), 'Soil type'] = 20  # sand
    else:
        pass
    if ndf2.empty == False:
        ndf2['#200 (%)'] = ndf2['#200 (%)'].fillna(method='ffill')
        ndf2['#200 (%)'] = ndf2['#200 (%)'].fillna(100)
        ndf2.loc[(ndf2['#200 (%)'] >= 50), 'Soil type'] = 0  # clay
        ndf2.loc[(ndf2['#200 (%)'] < 50), 'Soil type'] = 20  # sand
    else:
        pass
    ###############################################

    ################Number of layer calculation####################
    # For ST
    if ndf2.empty == False:
    ...
    if ndf.empty == False and ndf2.empty == False:
        ...
        ndf['PseudoGroup'] = y
        model2 = KMeans(n_clusters=SS_Layers, n_init=100)
    ...
    if ndf2.empty == False:
        ...
        for i in range(lastcluster):
            ax.axhline(y=st_array[i], color='r', linestyle='-')
    else:
        pass
    # For SS
    if ndf.empty == False and ndf2.empty == False:
        ...
        for i in range(lastcluster2 - lastcluster - 1):
            ax.axhline(y=ss_array[i], color='r', linestyle='-')
    elif ndf.empty == False and ndf2.empty == True:
        To1 = ndf.groupby(['Group'], sort=False)['To'].max() * -1
        ss_array = To1.to_numpy()
    ...
    if ndf2.empty == False and ndf.empty == False:
        ...
        for i in range(lastcluster):
            plt.text(0.25, -1 * ST_Params[i][0], i + 1)
        ...
        for i in range(lastcluster2 - lastcluster):
            plt.text(0.25, -1 * SS_Params[i][0], i + 1)
            ...
            plt.text(0.5, -1 * SS_Params[i][0], SS_Params[i][7])
    elif ndf2.empty == False and ndf.empty == True:
        ST_Params = ndf2.groupby(['Group'], sort=False)['To', 'Wn %', 'Avg. Su(t/sq.m.)', 'Avg. y (t/cu.m.)'].mean()
        ST_Params['Avg. Su(t/sq.m.)'] = ST_Params['Avg. Su(t/sq.m.)'] * 9.81
        np.round(ST_Params, 2)
        ...
        Params = ST_Params
        for i in range(lastcluster):
            plt.text(0.25, -1 * Params[i][0], i + 1)
            plt.text((1.5), -1 * Params[i][0], 'Su (kPa)  %.2f' % (Params[i][2]))
            plt.text((3), -1 * Params[i][0], 'y (t/cu.m.) %.2f' % (Params[i][3]))
            plt.text(0.5, -1 * Params[i][0], Params[i][4])
    else:
        SS_Params = ndf.groupby(['Group'], sort=False)[
            'To', 'Wn %', 'Su from Ncorr', 'Phi from Ncorr', 'Avg. y (t/cu.m.)', 'N'].mean()
        print(SS_Params)
